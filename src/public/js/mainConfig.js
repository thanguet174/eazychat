  ...
  $(`#write-chat-${divId}`).emojioneArea({
    ...
    events: {
      keyup: function(editor, event) {
        //gắn thay đổi giá trị vào ô input bị ẩn
...
function flashMasterNotify(){
  ...
  if (notify.length){
    alertify.notify(notify, "success", 2);
  }
}
...
function convertEmoji(){
  $(".convert-emoji").each(function() {
    ...
    var coverted = emojione.toImage(original);
    $(this).html(coverted);
  });
}
...
$(document).ready(function() {
  ...
  //hiện message ở màn hình master khi đăng nhập
  flashMasterNotify()
  //Thay đổi kiểu chat
  changeTypeChat();
  ...
  $("#video-chat-group").bind("click", function(){
    alertify.notify("Tính năng này không khả dụng với nhóm trò chuyện","error",3);
  });
